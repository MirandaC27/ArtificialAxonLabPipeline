# Step 1: Rename Keyence files for 3D assay data (multi-channel, dynamic via JSON)
...
# Enter Plate directory (1 level down)
...
cd "$PLATE_DIR" || { echo "Failed to cd into Plate directory"; exit 1; }


# Loop through well directories
ls -d W* > "$DIR0/dirlist"
...
do
  ...
  cd "$PLATE_DIR/$dirname" || { echo "Failed to cd into $dirname"; continue; }
  ...
  do
    ...
    cd "$PLATE_DIR/$dirname/$trackname" || { echo "Failed to cd into $trackname"; continue; }


    # Loop through channels from JSON
    ...
    do
        ...
        cp "$PLATE_DIR/$dirname/$trackname/$oldname" "$TRACKS1/$newname"
