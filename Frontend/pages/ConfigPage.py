...
class ConfigPage(tk.Frame):
    ...
    def render_config_list(self):
        ...
        for index, config in enumerate(self.configs):
            item = tk.Label(
                self.list_frame,
                text=f"→ {config.get('config_name', 'Unnamed')}",
                anchor="w",
                padx=10,
                pady=8,
                bg="white",
    ...
    def format_config_text(self, config):
        ...
        return "\n".join([
            ...
            "",
            f"A: {config.get('a', 'N/A')}",
            f"B: {config.get('b', 'N/A')}",
            f"Sum Result: {config.get('result', 'N/A')}",
            "",
            f"First Name: {config.get('first_name', 'N/A')}",
            f"Last Name: {config.get('last_name', 'N/A')}",
            f"Full Name: {config.get('full_name', 'N/A')}",
        ])
    ...
    def autofill_selected_config(self):
        ...
        number_data["a"] = self.selected_config["a"]
        number_data["b"] = self.selected_config["b"]

        name_data["first_name"] = self.selected_config["first_name"]
        name_data["last_name"] = self.selected_config["last_name"]

        messagebox.showinfo("Autofilled", "Config loaded into the form fields.")
